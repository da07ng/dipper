...
getPortalSelf(cookie('dipper_token')).then(response => {
  if (response.ok) {
    response.json().then(json => {
      ...
      let query = `orgid:${portalId} ${portalQueryConfig.viewQueries.web} ${portalQueryConfig.viewQueries.none} ${portalQueryConfig.filterQueries['tools'].f}`

      search(query, '', 9, 'modified', 'desc', cookie('dipper_token')).then(response => {
        if (response.ok) {
          response.json().then(json => {
...
function showMapList(functionList) {
  let functionListHtml = ''
  for(let i = 0; i < functionList.length; i++) {
    let map = functionList[i]
    ...
    functionListHtml += `<div class="col-md-4 map">
      <div class="thumbnail">
  ...
  $('.function-list').html(functionListHtml)
}