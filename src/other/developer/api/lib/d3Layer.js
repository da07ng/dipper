...
dojo.declare("modules.d3Layer", esri.layers.GraphicsLayer, {
    ...
    _bind: function(map){
      this._connects = [];
      this._connects.push( dojo.connect( window.map, "onZoomEnd", this, this._reset ) );
      this._connects.push( dojo.connect( window.map, "onPanEnd", this, this._reset ) );
    },

    _project: function(x){
       var p = new esri.geometry.Point( x[0], x[1] );
       var point = window.map.toScreen( esri.geometry.geographicToWebMercator( p ), window.map.spatialReference )
